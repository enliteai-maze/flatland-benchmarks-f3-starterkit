...
RUN conda env list
RUN /bin/bash -c "source activate /opt/conda/envs/flatland-baselines && pip install git+https://github.com/enlite-ai/maze.git@88dcd86134f8baae2f6da09034e45c3b435ef1ba#egg=maze-rl"
RUN /bin/bash -c "source activate /opt/conda/envs/flatland-baselines && pip install git+https://github.com/enlite-ai/maze-flatland.git@main"
RUN /bin/bash -c "source activate /opt/conda/envs/flatland-baselines && pip install xgboost && pip install scikit-learn && echo pip list"
